...
static void adc_setup(void)
{
	...
	adc_power_off(ADC1); //Turn off the ADC to reduce power consumption to a few microamps.
	adc_set_clk_source(ADC1, ADC_CLKSOURCE_ADC);
	adc_calibrate_start(ADC1);
	adc_calibrate_wait_finish(ADC1);
	adc_set_operation_mode(ADC1, ADC_MODE_SCAN); //In this mode, after the trigger event, all channels will be converted once, storing results sequentially.
	adc_disable_external_trigger_regular(ADC1); //ADC Disable an External Trigger for Regular Channels.
	adc_set_right_aligned(ADC1); //ADC Set the Data as Right Aligned.
	//adc_enable_temperature_sensor(); //This enables the sensor on channel 16
...
int main(void)
{
	...
	while (1) {
		...
		*/
		...
		while (!(adc_eoc(ADC1))); //This flag is set after all channels of a regular or injected group have been converted.
