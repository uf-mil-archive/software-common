...
Mat Thresholder::orange() {
	...
	Mat s; inRange(channelsHSV[1],Scalar(0,0,0,0),Scalar(70,0,0,0),s);
...
Mat Thresholder::red() {
    //Mat dbg; inRange(channelsRGB[2],Scalar(155,0,0,0),Scalar(255,0,0,0),dbg);
    //return dbg;
	Mat x; addWeighted(channelsRGB[0], 1/2., channelsRGB[1], 1/2., 255/2., x);
	Mat y; addWeighted(x, 1, channelsRGB[2], -1/2., 0, y);
	Mat dbg; adaptiveThreshold(y,dbg,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY_INV,35,2);
	return dbg;
}
